<!-- 7 module -->
    ...
    <!-- module-7-google-oauth-and-swagger ======================-->

1.  ✅ Реєстрація проєкту в Google Сloud Console (https://console.cloud.google.com/welcome/new?pli=1&inv=1&invt=AbqhtA);

    - створили проект Students app, Api & Services -> ...
      На виході отримуємо json файл (завантажуємо його). Переназиваємо на google-oauth.json.

    ??????? google-oauth.json вставляти в проект ?????

    🔵 google-oauth.json має чутливу інформацію. ✅ ВСЮ ЧУТЛИВУ ІНФОРМАЦІЮ переносимо у змінні оточення(env, env.example, env.js), щоб вони не потрапили до гітхабу. В env:
    /////
...
2.  Створення унікального посилання для додатку (за яким буде відбуватися аутентифікація користувача). Генерацію такого посилання:
    🟠 npm i google-auth-library - бібліотеку для роботи з авторизацією від Google
3.  створюємо googleOAuthClient і функцію для генерації посилання:
    // src/utils/googleOAuth2.js

    🔷 генеруємо URL, за яким користувач може авторизуватися, а потім передавати дозволи на доступ до своїх даних (електронної пошти та профілю).

    - Користувач заходить за наданим URL
    - надає Google доступ до своїх даних
    - Google перенаправляє користувача на зазначений redirectUri з кодом авторизації, який потім можна обміняти на токен доступу для доступу до даних користувача.

4.  // src/controllers/auth.js
5.  // src/routers/auth.js

6.  Створення логіну
    // src/utils/googleOAuth2.js
    // src/services/auth.js
    // src/controllers/auth.js
    // src/validation/auth.js - схема для валідації:
